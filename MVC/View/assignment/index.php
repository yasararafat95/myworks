                ...
                <?php if((($siteinfos->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) || ($this->session->userdata('usertypeID') != 3)) { ?>
                    <h5 class="page-header">
                        ...
                        <?php if($this->session->userdata('usertypeID') != 3) { ?>
                            <div class="col-lg-3 col-sm-4 col-md-4 col-xs-12 pull-right drop-marg">
                                <?php
                    ...
                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer">
                        <thead>
                            <tr>
								...
								<th><?=$this->lang->line('assignment_subject')?></th>
                        ...
                        <tbody>
							 ...
							 if($this->session->userdata('usertypeID') == 3)
							{
								$i = 1;
								$j=0;
								//print_r($section_assignment_data);
								foreach($section_assignment_data as $k=>$v)
								{
								//print_r($v);
									 ...
									 echo '<tr>';
										...
										<td>'.date('d-m-Y',strtotime($v->deadlinedate)).'</td>';
										?>
										<td data-title="<?=$this->lang->line('assignment_section')?>">
											<?php  
											if($v->sectionID == 'false') 
											{
											...
											{
												$dbSections = json_decode($v->sectionID);
												if(count($dbSections))
												{
													foreach ($dbSections as $dbSectionkey => $dbSection) 
													{
                                        ...
                                        <?=isset($teachers[$v->subjectID]) ? $teachers[$v->subjectID] : '' ?>
										</td>-->
										<td data-title="<?=$this->lang->line('teacher_name')?>">
										   <?php echo getNameByUsertypeIDAndUserID($v->usertypeID, $v->userID); ?>
										</td>
										<td data-title="<?=$this->lang->line('assignment_file')?>">
											<?php 
												if($v->originalfile) 
												{
													echo btn_download_file('assignment/download/'.$v->assignmentID, namesorting($v->originalfile), $this->lang->line('download')); 
												}
											?>
										</td>
										<?php if(permissionChecker('assignment_edit') || permissionChecker('assignment_delete') || permissionChecker('assignment_view')) { 
										...
										<?php echo btn_view('assignment/view/'.$v->assignmentID.'/'.$set, $this->lang->line('view')) ?>
											<?php
											if($this->session->userdata('usertypeID') == 3 ){ 
										//print_r($v->ansstatus);
											
											//echo "assgn status = ".$v->ansstatus."<br>";
										//	}
											?> 
												<?php
												if(!isset($v->ansstatus)){
													
													if($siteinfos->school_year == $this->session->userdata('defaultschoolyearID') ) { ?>
														<?php echo btn_upload('assignment/assignmentanswer/'.$v->assignmentID.'/'.$set, $this->lang->line('upload')); ?>
													<?php }
												}else{
													echo "Submitted";
												}													?>
											...
											<?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) { ?>
												<?php echo btn_edit('assignment/edit/'.$v->assignmentID.'/'.$set, $this->lang->line('edit')) ?>
												<?php echo btn_delete('assignment/delete/'.$v->assignmentID.'/'.$set, $this->lang->line('delete')) ?>
											<?php } ?>
										</td>
										<?php } ?>
									</tr>
							...
							//print_r($assignments);
							//if($this->session->userdata('usertypeID') == 1 || $this->session->userdata('usertypeID') == 2 || $this->session->userdata('usertypeID') == 7)
							else
							{
								if(count($assignments)) 
								{
									...
									foreach($assignments as $assignment) 
									{
										...
										<td data-title="<?=$this->lang->line('assignment_subject')?>">
										...
										<td data-title="<?=$this->lang->line('action')?>">
											<?php 
											//echo "assgn status = ".$assignmentanswer->status."<br>"; 
											if($this->session->userdata('usertypeID') == 3) { ?> 
